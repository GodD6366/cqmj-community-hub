...
export function McpConnectClient({ currentUser, endpoint, initialToken, welcome }: McpConnectClientProps) {
  ...
  const prompt = useMemo(() => buildMcpConnectionPromptForUser({ endpoint, apiKey: token, user: currentUser }), [currentUser, endpoint, token]);

  return (
    <PageShell className="max-w-[1500px] space-y-4">
      <section className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)_320px]">
        <CyberPanel title="MCP 接入" kicker="Connection Setup">
        ...
        <CyberPanel title="接入信息" kicker="Config">
          ...
          <div className="grid gap-4">
            ...
            <label className="grid gap-2 text-sm font-semibold text-slate-900">
              <span>接入文案</span>
              <TextArea aria-label="接入文案" fullWidth readOnly rows={14} value={prompt} variant="secondary" />
            </label>
          </div>
