      ...
      <div className="mobile-resident-only mobile-resident-stack">
        ...
        >
          <ReactMarkdown
            ...
            components={{
              h1: ({ children }) => (
                <h1 className="editorial-title mb-6 text-[2rem] leading-tight font-semibold text-slate-950">
                  {children}
                </h1>
              ),
              h2: ({ children }) => (
                <h2 className="mt-8 mb-4 border-t border-[var(--border)] pt-5 text-[1.15rem] font-semibold tracking-tight text-slate-950">
                  {children}
                </h2>
              ),
              h3: ({ children }) => <h3 className="mt-7 mb-3 text-base font-semibold text-slate-900">{children}</h3>,
              h4: ({ children }) => <h4 className="mt-5 mb-3 text-sm font-semibold text-slate-900">{children}</h4>,
              h5: ({ children }) => <h5 className="mt-4 mb-2 text-sm font-semibold text-slate-900">{children}</h5>,
              h6: ({ children }) => (
                <h6 className="mt-4 mb-2 text-[0.72rem] font-semibold uppercase tracking-[0.18em] text-slate-600">{children}</h6>
              ),
              p: ({ children }) => <p className="my-4 text-[0.95rem] leading-7 text-slate-700">{children}</p>,
              ul: ({ children }) => (
                <ul className="my-4 list-disc space-y-2 pl-5 text-[0.95rem] leading-7 text-slate-700 marker:text-[var(--accent)]">
                  {children}
                </ul>
              ),
              ol: ({ children }) => (
                <ol className="my-4 list-decimal space-y-2 pl-5 text-[0.95rem] leading-7 text-slate-700 marker:font-medium marker:text-[var(--primary)]">
                  {children}
                </ol>
              ),
              ...
              pre: ({ children }) => (
                <pre className="my-6 overflow-x-auto rounded-[1.35rem] bg-[#1c2c3f] p-4 text-xs leading-6 text-slate-100">
                  {children}
                </pre>
              ),
              blockquote: ({ children }) => (
                <blockquote className="my-6 rounded-[1.15rem] border border-[var(--border)] bg-[linear-gradient(180deg,var(--primary-soft),rgba(255,255,255,0.95))] px-4 py-4 text-[0.95rem] leading-7 text-slate-700">
                  {children}
                </blockquote>
              ),
